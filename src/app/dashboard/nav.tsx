...
const ITEMS = [
  { href: "/dashboard/invites", label: "Convites digitais", hint: "Templates e criar convite", icon: Mail },
  { href: "/dashboard", label: "Eventos", hint: "Os seus eventos e convidados", icon: CalendarHeart },
];
      ...
      {ITEMS.map((it) => {
        ...
        return (
          <Link
            key={it.href}
            href={it.href}
            className={`flex items-center gap-3 whitespace-nowrap rounded-full px-4 py-3 text-sm transition ${active ? "bg-brand-700 text-white shadow-[0_6px_18px_rgba(84,27,56,0.22)]" : "text-brand-800 hover:bg-brand-100"}`}
          >
