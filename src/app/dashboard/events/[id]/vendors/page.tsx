    ...
    <>
      ...
      <div className={`grid gap-6 ${plan.canPlan ? "lg:grid-cols-[1fr_2fr]" : ""}`}>
        ...
        <details className="card" open={vendors.length === 0}>
          ...
          <div><label className="label">Nome</label><input name="name" className="input" required placeholder="Quinta da Serra, DJ Marco…" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Contacto</label><input name="contactName" className="input" placeholder="Pessoa" /></div>
            ...
            <div><label className="label">Site / Instagram</label><input name="website" type="url" className="input" placeholder="https://" /></div>
          </div>
        ...
        <div className="space-y-4">
          ...
          {[...byCategory.entries()].map(([cat, list]) => (
            <div key={cat} className="card">
              <h2 className="font-semibold">{cat} <span className="text-xs font-normal text-[#8c7b87]">· {list.length} proposta(s)</span></h2>
