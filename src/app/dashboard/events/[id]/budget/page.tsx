...
export default async function BudgetPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return (
    <>
      ...
      {contributions > 0 && <p className="mt-3 text-sm text-muted">Contribuições recebidas na lista de presentes: <strong className="text-emerald-800">{formatMoney(contributions, cur)}</strong>.</p>}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_2fr]">
        <div className="space-y-6">
          ...
          <details className="card" open={items.length === 0}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label" htmlFor="b-category">Categoria</label><select id="b-category" name="category" className="input">{BUDGET_CATEGORIES.map((c) => <option key={c}>{c}</option>)}</select></div>
              <div><label className="label" htmlFor="b-vendorId">Fornecedor</label><select id="b-vendorId" name="vendorId" className="input"><option value="">—</option>{vendors.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</select></div>
            </div>
            <div><label className="label" htmlFor="b-name">Item</label><input id="b-name" name="name" className="input" required placeholder="Aluguer do espaço, menu por pessoa…" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label" htmlFor="b-estimated">Estimado ({cur})</label><input id="b-estimated" name="estimated" className="input" inputMode="decimal" placeholder="3000" /></div>
              <div><label className="label" htmlFor="b-contracted">Contratado ({cur})</label><input id="b-contracted" name="contracted" className="input" inputMode="decimal" placeholder="quando fechar" /></div>
            </div>
                ...
                {upcoming.map((p) => {
                  ...
                  return (
                    <li key={p.id} className="flex justify-between gap-2">
                      ...
                      <span className={days < 0 ? "font-semibold text-red-700" : days <= 7 ? "text-amber-800" : "text-muted"}>{formatMoney(p.amount, cur)} · {formatEventDate(p.dueAt!, false, event.timezone)}</span>
                    </li>
                  );
                })}
              </ul>
        ...
        <div className="space-y-4">
          {items.length === 0 && <p className="card text-sm text-muted">Comece por estimar as grandes rubricas (local, catering, fotografia). Quando contratar um fornecedor no separador Fornecedores, o valor entra aqui automaticamente.</p>}
            ...
            return (
              <div key={cat} className="card">
                <div className="flex items-baseline justify-between">
                  ...
                  <span className="text-xs text-muted">estimado {formatMoney(catEst, cur)} · contratado {formatMoney(catCon, cur)}</span>
                </div>
                    ...
                    return (
                      <li key={i.id} className="py-3">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div>
                            <p className="font-medium">{i.name}{i.vendor && <span className="ml-2 text-xs text-muted">· {i.vendor.name}</span>}</p>
                            <p className="text-xs text-muted">pago {formatMoney(itemPaid, cur)} de {formatMoney(total, cur)}</p>
                          ...
                          <ul className="mt-2 space-y-1 text-xs">
                            {i.payments.map((p) => (
                              <li key={p.id} className="flex items-center gap-2">
                                ...
                                <span className={p.paidAt ? "text-muted line-through" : ""}>{p.label ?? "Pagamento"} · {formatMoney(p.amount, cur)}{p.dueAt ? ` · ${formatEventDate(p.dueAt, false, event.timezone)}` : ""}</span>
                                <form action={deletePaymentAction.bind(null, id, p.id)}><ConfirmButton className="text-muted hover:text-red-700" message={`Remover o pagamento de ${formatMoney(p.amount, cur)}?`}><X className="h-3 w-3" aria-label="Remover pagamento" /></ConfirmButton></form>
                              </li>
                            ))}
                          </ul>
                        )}
                        ...
                        <form action={deleteBudgetItemAction.bind(null, id, i.id)} className="mt-1 text-right"><ConfirmButton className="text-xs text-muted hover:text-red-700" message={`Remover "${i.name}"${i.payments.length ? ` e os seus ${i.payments.length} pagamento(s)` : ""}?`}>remover item</ConfirmButton></form>
                      </li>
                    );
                  })}
                </ul>
