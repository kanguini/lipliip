...
export function TemplateCard({ t, type, names, selected }: { t: TemplateMeta; type: string; names?: string; selected: boolean }) {
  const kicker = EVENT_TYPES[type as EventType]?.label ?? "Evento";
  return (
    <div className={`overflow-hidden rounded-xl border bg-white transition ${selected ? "border-brand-500 ring-2 ring-brand-200" : "border-brand-200/70 hover:border-brand-300"}`}>
      {t.collection === "2026" ? (
