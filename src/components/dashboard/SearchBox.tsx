...
function SearchInput() {
  ...
  return (
    <input key={query} id="global-search" name="q" type="search" defaultValue={query} style={{ paddingLeft: "2.75rem" }} className="input bg-brand-50 py-2 shadow-none focus:bg-white" placeholder="Pesquisar eventos, convidados…" maxLength={80} autoComplete="off" />
  );
}
...
export function SearchBox() {
  return (
    <form action="/dashboard/search" method="get" role="search" className="relative min-w-0 flex-1 sm:max-w-md">
      <label htmlFor="global-search" className="sr-only">Pesquisar eventos e convidados</label>
