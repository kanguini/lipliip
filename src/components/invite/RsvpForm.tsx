...
export function RsvpForm({ token, maxCompanions, allowChildren, current, deadlinePassed, deadlineLabel, songRequests }: Props) {
  ...
  const [companions, setCompanions] = useState(String(current.companions));
      ...
      {deadlinePassed ? (
        ...
        <form action={formAction} className="mt-4 space-y-4">
          <fieldset className="grid grid-cols-2 gap-2">
            <legend className="sr-only">Vai estar presente?</legend>
            {[
              ["ACCEPTED", "Sim, vou!"],
              ...
              <label
                key={v}
                className="cursor-pointer rounded-xl border-2 px-4 py-3 text-center text-sm font-semibold transition has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-[var(--inv-accent)] has-[:focus-visible]:ring-offset-2"
                style={{
          ...
          {status === "ACCEPTED" && (
            <>
              {maxCompanions > 0 && (
                <div>
                  <label htmlFor="rsvp-companions" className="mb-1 block text-sm font-medium">Acompanhantes (até {maxCompanions})</label>
                  <select id="rsvp-companions" name="companions" className="invite-input" value={companions} onChange={(e) => setCompanions(e.target.value)}>
                    {Array.from({ length: maxCompanions + 1 }).map((_, i) => (
                      <option key={i} value={i}>{i === 0 ? "Vou sozinho/a" : `+${i}`}</option>
                    ))}
                  </select>
                  <input name="companionNames" aria-label="Nomes dos acompanhantes" className="invite-input mt-2" placeholder="Nomes dos acompanhantes" defaultValue={current.companionNames ?? ""} />
                  {!allowChildren && <p className="mt-1 text-xs opacity-70">Este evento é apenas para adultos.</p>}
                </div>
              )}
              {songRequests && (
                <div>
                  <label htmlFor="rsvp-song" className="mb-1 flex items-center gap-1.5 text-sm font-medium"><Music className="invite-accent h-4 w-4" aria-hidden />Que música não pode faltar na festa?</label>
                  <input id="rsvp-song" name="songRequest" className="invite-input" placeholder="Artista - Música" defaultValue={current.songRequest ?? ""} />
                </div>
              )}
              <div>
                <label htmlFor="rsvp-diet" className="mb-1 block text-sm font-medium">Restrições alimentares ou alergias</label>
                <input id="rsvp-diet" name="dietaryNotes" className="invite-input" placeholder="Ex: vegetariano, sem glúten" defaultValue={current.dietaryNotes ?? ""} />
              </div>
            </>
          )}
          <div>
            <label htmlFor="rsvp-message" className="mb-1 block text-sm font-medium">Mensagem para os anfitriões (opcional)</label>
            <textarea id="rsvp-message" name="rsvpMessage" className="invite-input" rows={2} defaultValue={current.rsvpMessage ?? ""} />
          </div>
