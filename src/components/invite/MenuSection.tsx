                ...
                <li key={idx} className="text-sm">
                  ...
                  {i.tags && i.tags.length > 0 && (
                    <span className="ml-2 inline-flex flex-wrap gap-1 align-middle">
                      {i.tags.map((t) => (
                        <span key={t} className="rounded-full px-2 py-0.5 text-[10px] uppercase tracking-wide" style={{ background: "color-mix(in srgb, var(--inv-accent) 14%, transparent)" }}>{t}</span>
                      ))}
                    </span>
                  )}
                </li>
